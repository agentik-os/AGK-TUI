...
import argparse,ast,fcntl,json,os,pwd,re,signal,subprocess,sys,time
...
def _lock_path(user:str)->Path:
 return Path('/run/agk-station')/f'safe-reload-{user}.lock'

def acquire_reload_lock(user:str):
 Path('/run/agk-station').mkdir(mode=0o755, parents=True, exist_ok=True)
 lock=open(_lock_path(user),'w')
 try:
  fcntl.flock(lock.fileno(), fcntl.LOCK_EX | fcntl.LOCK_NB)
 except OSError:
  lock.close()
  print(json.dumps({'status':'already-in-progress','user':user}))
  raise SystemExit(0)
 return lock
...
def main()->int:
 ...
 lock=acquire_reload_lock(args.user)
 ...
 # AGK nodrain: never reload while draining/restarting (Discord: Gateway is restarting)
 if initial.get('gateway_state') in {'draining','restarting','starting'} or initial.get('restart_requested'):
  print(json.dumps({'status':'busy','user':args.user,'unit':args.unit,'active_agents':initial.get('active_agents'),'gateway_state':initial.get('gateway_state'),'restart_requested':bool(initial.get('restart_requested')),'reason':'already-draining'}))
  return 0
 if int(initial.get('active_agents') or 0)!=0:
  print(json.dumps({'status':'busy','user':args.user,'unit':args.unit,'active_agents':initial['active_agents'],'reason':'live-turn-no-reload'}))
  return 0
 ...
 try:
  while time.monotonic()<deadline:
   ...
   if current['active_agents']!=0:
    print(json.dumps({'status':'busy','user':args.user,'unit':args.unit,'active_agents':current['active_agents'],'reason':'live-turn-no-reload'}))
    return 0
   marker_active=True
