...
def _embed(discord_module, records, selected=None, page=0):
    ...
    embed = discord_module.Embed(
        ...
        color=discord_module.Color.from_rgb(17, 17, 17),
    )
...
class StationSessionView(discord.ui.View if discord else object):
    ...
    def _build(self):
        ...
        buttons = [
            ("Refresh", discord.ButtonStyle.secondary, self._refresh, False),
...
def register_station_session_commands(adapter, tree) -> bool:
    ...
    async def station_sessions(interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.edit_original_response(content=f"Session catalog unavailable: {exc}", embed=None, view=None)
            return
        await interaction.edit_original_response(embed=_embed(discord, view.records, None, 0), view=view)
