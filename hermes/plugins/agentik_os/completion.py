...
_PLANNED_TURNS: "OrderedDict[str, None]" = OrderedDict()
_PLAN_LOCK=threading.Lock()
_PLAN_TURN_LIMIT=2048


def require_plan_before_work(tool_name:str="",args:dict|None=None,turn_id:str="",**_kwargs):
 """Block operational tools until the current turn has applied a todo plan."""
 ...
 if name=="todo":
  return None
 if name in _PLAN_EXEMPT_TOOLS:
  return None
 with _PLAN_LOCK: planned=turn in _PLANNED_TURNS
 if planned:
  return None
...
def record_applied_plan(tool_name:str="",turn_id:str="",result:Any=None,status:str="",**_kwargs):
 """Authorize work only after a successful canonical todo result is persisted."""
 if str(tool_name or "").strip()!="todo" or str(status or "").lower() not in {"ok","success"}: return None
 turn=str(turn_id or "").strip()
 if not turn or not _canonical_plan_from_result(result): return None
 with _PLAN_LOCK:
  _PLANNED_TURNS[turn]=None; _PLANNED_TURNS.move_to_end(turn)
...
def completion_prompt(_session_info: dict | None = None):
 return ("AGK Completion Harness is active. The original user prompt is archived before execution. For every non-trivial request, apply a canonical todo plan before operational work: enumerate every currently known action upfront, including pending verification, deployment, evidence, approval and rollback steps. Then use agk_completion to create a mission and persist every explicit/implicit requirement, constraint, deliverable, approval gate and committed follow-up. The Discord Station action message is projected only from that canonical plan; its first visible version must expose the full known checklist, keep exactly one item in_progress, update statuses after verified transitions, and revise the visible plan when scope changes. Attach artifacts and PASS evidence per requirement. Before saying done, call gate; continue the Gauntlet/Loop-Graph until permit_done=true. Never start newly discovered backlog work without explicit human authorization.")
