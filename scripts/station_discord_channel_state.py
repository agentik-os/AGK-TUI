...
def lease_active_count(home: Path, now: float) -> int:
    ...
    try:
        ...
        try:
            ...
            con.close()  # explicit close: Python 3.14 leaks 2 fds/tick otherwise
    except Exception:
        return 0
...
class ChannelStateProjector:
    ...
    def tick(self, *, now: float | None = None) -> str | None:
        ...
        if not isinstance(active_agents, int) or active_agents < 0:
            raise RuntimeError("gateway state unavailable")
        leases = lease_active_count(self.gateway_state_path.parent, timestamp)
        if leases > 0:
            active_agents = max(active_agents, leases)
