...
class DiscordClient:
    ...
    def create_thread(self, parent_channel_id: str, name: str, starter_content: str | None = None) -> str:
        ...
        # Text-channel public threads cannot embed message on create; create then fill immediately.
...
class Dispatcher:
    ...
    def dispatch(self, raw_record: dict) -> dict:
        ...
        if not thread_id:
            # ALWAYS create a new Discord thread per inter-agent mission.
            # AGK_THREAD_CREATE_AUTO_WAKE_V1: create + non-empty starter in same path.
            # Cross-face @mention is the real wake (never self).
            content = "\n".join([
            ...
            # Temporary starter so create never returns empty; replaced/augmented by handoff mention.
            thread_id = self.discord.create_thread(
                self.parent_channel_id,
                thread_title(record["source"], record["body"]),
                starter_content=(
                    f"Opening inter-agent handoff {record['source']}→{record['target']} "
                    f"`{record['id']}` — wake follows."
                ),
            )
            ...
            # Verify wake surface is non-empty after cross-face mention.
            meta = self.discord.get_channel(thread_id)
            count = int(meta.get("message_count") or meta.get("total_message_sent") or 0)
            if count < 1 or not thread_message_id:
                raise DispatchError(
                    "AGK_THREAD_CREATE_AUTO_WAKE_V1 FAILED wake: "
                    f"thread={thread_id} message_count={count}"
                )
            ...
            if count < 1:
                raise DispatchError(
                    f"AGK_THREAD_CREATE_AUTO_WAKE_V1 FAILED wake: thread={thread_id} message_count={count}"
                )
